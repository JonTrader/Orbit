...
- Invite continuation: validated local `/accept-invite?token=...` only (`parseLocalContinuation`). Post-auth callbacks go through `/` so Personal Space onboarding runs, then return to the Invite. After `acceptInvite`, redirect with the write result's `spaceId` - do not re-read memoized membership in the same request. Auth layout and `/` export `referrer: "no-referrer"` so Invite tokens in `continue` do not leak via Referer.
