...
- `components/spaces/` owns the Active Space chrome and rows:
  ...
  - Route group: `app/(app)/spaces/(active)/` wraps Active Space routes so the sidebar persists across `spaceId` changes. `/spaces` (`page.tsx` / `loading.tsx`) stays outside `(active)`. `(active)/loading.tsx` is the main-panel skeleton (header + tabs + content) shown as `{children}` inside `SpaceSidebarShell`; it must not include a sidebar, because the layout already rendered one. Section `loading.tsx` files only cover the content panel after `[spaceId]` chrome is mounted.
