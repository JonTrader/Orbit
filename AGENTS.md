...
- `lib/spaces/queries/` - pure RSC content SELECTs (no auth). Prefer these after `getActiveSpace` on Space pages. `fetchUpcomingTimeline` is the Upcoming page's one content query (`UNION ALL` of Monthlies + dated open Tasks); Section names come from layout `sections` in memory, not a JOIN. Keep `fetchTasks` / `fetchMonthlies` / `fetchNotes` for Daily, Monthlies, custom sections, and for services wrapping API routes.
